...
def get_file_prefix(filename):
    # Match the format of the name: e.g., name.12345.png
...
def predict():
    ...
    if 'audio' in request.form['fileType'] and file_ext in ['.wav', '.mp3', '.flac']:
        ...
        # Use trimmed audio file for playback
    ...
    # Handle image file upload
        ...
        # Extract the prefix (up to 5 digits) from the spectrogram filename
        spectrogram_prefix = get_file_prefix(filename)
        if not spectrogram_prefix:
            return "Invalid file name format. The name should be like 'name.12345.png'."

        # Construct the corresponding audio filename (with .wav extension)
        audio_filename = f"{spectrogram_prefix}.wav"
        audio_file_path = os.path.join(app.config['UPLOAD_FOLDER'], audio_filename)

        # Debugging: print out the paths to check
        print(f"Spectrogram prefix: {spectrogram_prefix}")
        print(f"Expected audio file path: {audio_file_path}")
